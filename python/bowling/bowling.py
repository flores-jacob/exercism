...
class BowlingGame(object):
    ...
    def roll(self, pins):
        ...
        if self.current_frame > FINAL_FRAME:
        ...
        elif (pins == 10) and (self.roll_count == 2) and (self.current_frame == FINAL_FRAME):
            self.current_roll = STRIKE
        else:
            self.current_roll = OPEN

        # For frames 0 to 8, and the first roll of FINAL_FRAME, the scoring is the same
        if (self.current_frame < FINAL_FRAME) or ((self.current_frame == FINAL_FRAME) and (self.roll_count == 1)):
            ...
            if self.previous_roll == SPARE:
                ...
                self.frames[self.current_frame - 1] += pins
        # However, on the second roll of FINAL_FRAME, the scoring is different
        elif (self.current_frame == FINAL_FRAME) and (self.roll_count == 2):
            if self.the_roll_before == STRIKE:
                self.frames[self.current_frame - 1] += pins

        if (self.current_frame != FINAL_FRAME) and (self.current_roll == STRIKE):
        ...
        # Edge cases for the final bonus roll in the final frame
        elif self.current_frame == FINAL_FRAME:
            if self.roll_count == 3:
                if (self.the_roll_before != STRIKE) and (self.previous_roll != SPARE):
                    ...
                    remaining_pins = self.frames[FINAL_FRAME] - 10 - pins
